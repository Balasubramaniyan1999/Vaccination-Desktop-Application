...
connector = sqlite3.connect('Vaccination.db')
cursor = connector.cursor()


connector.execute(
"CREATE TABLE IF NOT EXISTS Vaccination_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, AADHAR TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, CITY TEXT)"
)
...
def reset_fields():
    ...
    for i in ['name_strvar', 'aadhar_strvar', 'contact_strvar', 'gender_strvar','city_strvar']:
        exec(f"{i}.set('')")
...
def display_records():
    ...
    curr = connector.execute('SELECT * FROM Vaccination_MANAGEMENT')
    data = curr.fetchall()
...
def add_record():
    global name_strvar, aadhar_strvar, contact_strvar, gender_strvar,dob, city_strvar
    ...
    DOB = dob.get_date()
    city = city_strvar.get()


    if not name or not aadhar or not contact or not gender or not DOB or not city:
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
            'INSERT INTO Vaccination_MANAGEMENT (NAME, AADHAR, PHONE_NO, GENDER,DOB, CITY) VALUES (?,?,?,?,?,?)', (name, aadhar, contact, gender, DOB, city)
            )
            connector.commit()
...
def remove_record():
    if not tree.selection():
        ...
        connector.execute('DELETE FROM Vaccination_MANAGEMENT WHERE STUDENT_ID=%d' % selection[0])
        connector.commit()
...
def view_record():
    ...
    name_strvar.set(selection[1]); aadhar_strvar.set(selection[2])
    contact_strvar.set(selection[3]); gender_strvar.set(selection[4]);
    dob.set_date(date); city_strvar.set(selection[6])


# Initializing the GUI window
main = Tk()
main.title('Vaccination System')
...
cf_bg = 'DarkGreen' # bg color for the center_frame
...
city_strvar = StringVar()


# Placing the components in the main window
Label(main, text="Vaccination Process", font=headlabelfont, bg='white').pack(side=TOP, fill=X)
...
Label(left_frame, text="Aadhar", font=labelfont, bg=lf_bg).place(relx=0.2, rely=0.31)
...
Label(left_frame, text="City", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.7)
...
Entry(left_frame, width=19, textvariable=aadhar_strvar, font=entryfont).place(x=20, rely=0.36)
Entry(left_frame, width=19, textvariable=city_strvar, font=entryfont).place(x=20, rely=0.75)
...
Label(right_frame, text='Vaccinated Records', font=headlabelfont, bg='Purple', fg='LightCyan').pack(side=TOP, fill=X)

tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                    columns=('Student ID', "Name", "Aadhar", "Contact Number", "Gender", "Date of Birth", "City"))
